...
SLog SVMesh2D::loadVertices(std::vector<SVVertex2D> *vertices, std::vector<uint32_t> *indices,
                            VkPhysicalDevice *physicalDevice) {
    _amountOfVertices = static_cast<uint32_t>(vertices->size());
    VkDeviceSize bufferSize = sizeof(SVVertex2D) * _amountOfVertices;
...
void SVMesh2D::draw(VkCommandBuffer *commandBuffer) {
    vkCmdDrawIndexed(*commandBuffer, _amountOfIndices, 1, 0, 0, 0);
}

void SVMesh2D::bind(VkCommandBuffer *commandBuffer) {
    VkDeviceSize offsets[] = {0};
    vkCmdBindVertexBuffers(*commandBuffer, 0, 1, &_vertexBuffer, offsets);
    vkCmdBindIndexBuffer(*commandBuffer, _indexBuffer, 0, VK_INDEX_TYPE_UINT32);
}

std::vector<VkVertexInputAttributeDescription> SVVertex2D::getAttributeDescriptions() {
...
std::vector<VkVertexInputBindingDescription> SVVertex2D::getBindingDescriptions() {
    std::vector<VkVertexInputBindingDescription> bindingDescriptions(1);
    bindingDescriptions[0].binding = 0;
    bindingDescriptions[0].stride = sizeof(SVVertex2D);
